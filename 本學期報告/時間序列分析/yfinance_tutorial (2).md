...
### 🧠 基礎練習
1. 使用 `yfinance` 下載 2023 年的 **AAPL** 股價資料，繪製收盤價折線圖。
2. 計算 AAPL 的 20 日與 60 日移動平均線，並比較兩者交叉點。
3. 嘗試印出 AAPL 的公司資訊 (`ticker.info`) 並找出其市值。

### 💪 進階挑戰
4. 撰寫程式比較 **TSMC**、**ASML**、**NVDA** 三檔股票的年度報酬率。
5. 將前述比較結果以 **Streamlit** 儀表板呈現（折線圖 + 表格統計）。
6. 擴充 Streamlit 應用：
   - 增加選項讓使用者選擇顯示 RSI 或 MACD。
   - 加入「自動更新」功能，每 5 分鐘重新抓取資料。

### 🧭 延伸思考
7. 若要分析台灣上市櫃公司（如 2330.TW），yfinance 抓取方式有何不同？
8. 試著以 `Plotly` 或 `Altair` 替代 `matplotlib`，建立互動圖表。
9. 如何結合 `pandas-ta` 套件進行技術指標分析？
10. 將結果輸出為 CSV 並自動寄送至 Email，有哪些實作方式？
