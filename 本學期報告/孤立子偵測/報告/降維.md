...
# Ensure all columns in X are numeric, coercing errors
for col in X.columns:
    X[col] = pd.to_numeric(X[col], errors='coerce')

# Fill any NaN values that resulted from the coercion with the mean of each column
X = X.fillna(X.mean())
...
if len(df) > N_SAMPLE:
    ...
    # Pre-process X_sample in the same way as X
    X_sample = df_sample.drop(columns=["Class"])
    for col in X_sample.columns:
        X_sample[col] = pd.to_numeric(X_sample[col], errors='coerce')
    X_sample = X_sample.fillna(X.mean()) # Use the mean from the original X
